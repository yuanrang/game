...
;
        ...
        public MsgInfo<string> Authorizationlanguage(string OpenId, string BotId, string UlangArray, string UlangDetailsArray)
        {
            ...
            var ips = YRHelper.GetClientIPAddress(HttpContext);
            var ulang = db.Queryable<ULang>().Where(d => d.ManageUserPassportId == 502423741 && ulangarrays.Contains(d.Id.ToString())).Select(x => new ULang()
            {
                Ip = ips,
                AddTime = DateTime.Now,
                IsValid = 1,
            ...
            var insertulang = ulangdeials.Select(x => new ULangDetails()
            {
                ...
                AddTime = DateTime.Now,
                Ip = ips,
                IsKeyboard = x.IsKeyboard,
                ULangMark = x.ULangMark,
                IsValid = x.IsValid,
                ParentId = x.ParentId,
                ULangId = x.ULangId,
                ULangValue = x.ULangValue,
                UpdateTime = DateTime.Now
            }).ToList();
            try
            {
        ...
        /// </summary>
        /// <param name="OpenId"></param>
        /// <param name="Id">信的id</param>
        /// <returns></returns>
        [HttpGet]
        public object GetDraftbox(string OpenId, int Id)
        {
            MsgInfo<string> info = new();
            long _PassportId = IsLogin(OpenId);
            if (_PassportId <= 0)
          ...
          var msg=  db.Queryable<CommunicationRecords>().First(d => d.ManageUserPassportId == _PassportId&&d.Id==Id).MsgImg;
            using (var sw = new FileStream(msg, FileMode.Open))
            {
                var bytes = new byte[sw.Length];
                sw.Read(bytes, 0, bytes.Length);
                sw.Close();
        ...
        /// <returns></returns>
        [HttpGet]
        public MsgInfo<RtnCommunRecords> EditCoomRecords(string OpenId,int Id)
        {
            MsgInfo<RtnCommunRecords> info = new MsgInfo<RtnCommunRecords>();
            long _PassportId = IsLogin(OpenId);
            if (_PassportId <= 0)
            {
                info.code = 400;
            ...
            var msg = db.Queryable<CommunicationRecords>().First(d => d.ManageUserPassportId == _PassportId && d.Id == Id);
            
            info.data = new RtnCommunRecords() { Body=msg.MsgBody, id=msg.Id, Title=msg.MsgTitle, ToolName=db.Queryable<CommunicationTools>().First(x=>x.Id== msg.ToolsId).CommunicationName, UserName=msg.PassportId};
            return info;
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<object> StandInsideLetter()
        {

            string OpenId = string.Empty; int id = 0; int ToolsId = 0; string Title = string.Empty; string PassportId = string.Empty; string Body = string.Empty; IFormFile file = null; int status = 0;
            ...
            foreach (var item in httpvalue)
            {
                ...
                if (item.Key == nameof(id))
                    int.TryParse(item.Value, out id);

                if (item.Key == nameof(ToolsId))
                    int.TryParse(item.Value, out ToolsId);

                Title = (item.Key == nameof(Title)) ? item.Value : Title;

                PassportId = (item.Key == nameof(PassportId)) ? item.Value : PassportId;

                Body = (item.Key == nameof(Body)) ? item.Value : Body;

                if (item.Key == nameof(status))
                    int.TryParse(item.Value, out status);
            }
